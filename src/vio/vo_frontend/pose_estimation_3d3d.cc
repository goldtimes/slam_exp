...
class EdgeProjectXYZRGBDPoseOnly : public g2o::BaseUnaryEdge<3, Eigen::Vector3d, VertexPose> {
   public:
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW;
    EdgeProjectXYZRGBDPoseOnly(const Eigen::Vector3d &pos3d) : pos3d(pos3d) {
    }

    virtual void computeError() override {
        const VertexPose *pose = static_cast<const VertexPose *>(_vertices[0]);
        _error = _measurement - pose->estimate() * pos3d;
    }
    virtual void linearizeOplus() override {
        // derror / dT的求导
        VertexPose *v = static_cast<VertexPose *>(_vertices[0]);
        Sophus::SE3 T = v->estimate();
        Eigen::Vector3d xyz_tran = T * pos3d;
        // de / dr
        _jacobianOplusXi.block<3, 3>(0, 0) = -Eigen::Matrix3d::Identity();
        // de / dt
        _jacobianOplusXi.block<3, 3>(0, 3) = Sophus::SO3::hat(xyz_tran);
    }
...
void BA3DTO3DG2O(const vector<Point3f> &pts1, const vector<Point3f> &pts2, Mat &R, Mat &t);
...
int main(int argc, char **argv) {
    ...
    std::string im1_depth_path = "/home/hang/robot_algo_ws/src/slam_exp/img_data/1_depth.png";
    std::string im2_depth_path = "/home/hang/robot_algo_ws/src/slam_exp/img_data/2_depth.png";
    ...
    // 建立3D点
    Mat depth1 = imread(im1_depth_path, CV_LOAD_IMAGE_UNCHANGED);  // 深度图为16位无符号数，单通道图像
    Mat depth2 = imread(im2_depth_path, CV_LOAD_IMAGE_UNCHANGED);  // 深度图为16位无符号数，单通道图像
...
void pose_estimation_3d3d(const vector<Point3f> &pts1, const vector<Point3f> &pts2, Mat &R, Mat &t) {
    ...
    p1_center = Point3f(Vec3f(p1_center) / N);
    p2_center = Point3f(Vec3f(p2_center) / N);

    vector<Point3f> q1(N), q2(N);  // 减去质心的点集
    for (int i = 0; i < N; ++i) {
    ...
    Eigen::Vector3d t_ = Eigen::Vector3d(p1_center.x, p1_center.y, p1_center.z) -
                         R_ * Eigen::Vector3d(p2_center.x, p2_center.y, p2_center.z);
    // convert to cv::Mat
